...
export default function ProductsPage() {
  ...
  const BUNDLES_DEFINITIONS = [
    {
      id: 'motorrax-coleccion-naranja-2026',
      title: 'MOTORRAX Colección Naranja 2026',
      subtitle: 'COLECCIÓN OFICIAL EXCLUSIVA (10% OFF)',
      description: 'Equípate con la colección naranja 2026 completa: Jersey MOTORRAX (+Buff), Gorra Oficial, Rompevientos y Termo Botella Acero Inoxidable 32 Oz.',
      discountPercentage: 10,
      productIds: [
        'db18cf9c-f5c7-42e5-b4b1-ea37fc6eb6c0', // Jersey MOTORRAX
        '4aa21594-88f9-4364-a592-45ebc8afc521', // Gorra Oficial
        '39a8d102-aa52-4ff9-a75f-7ebc211ea5a3', // Rompevientos
        '2494d3f8-48e7-4890-ad1c-02d4025ec280', // Termo Botella
      ],
    },
  ];

  const [inspectingBundle, setInspectingBundle] = useState(null);
  const [showBundleModal, setShowBundleModal] = useState(false);
  const [bundleSalesData, setBundleSalesData] = useState([]);
  const [loadingBundleSales, setLoadingBundleSales] = useState(false);

  const [salesCounts, setSalesCounts] = useState({});
  const [epSalesRaw, setEpSalesRaw] = useState([]);
  ...
  const fetchData = async () => {
    try {
      ...
      const [
        ...
        { data: epProdData, error: epProdErr },
        { data: profProdData, error: profProdErr },
      ] = await Promise.all([
        supabase.from('products').select('*').order('created_at', { ascending: false }),
        supabase.from('routes').select('id, title').order('id', { ascending: false }),
        supabase.from('route_products').select('*'),
        supabase.from('event_profile_product').select(`
          id, product_id, quantity, unit_price_cents, notes, created_at,
          event_profile (
            id, full_name, payment_status, participant_number, name_on_jersey,
            profile:profiles ( id, name, email )
          )
        `),
        supabase.from('profile_product').select('product_id, quantity'),
      ]);
      ...
      setEpSalesRaw(epProdData || []);

      // Compute total units sold per product
      const counts = {};
      (epProdData || []).forEach(item => {
        let notesObj = {};
        if (typeof item.notes === 'string') {
          try { notesObj = JSON.parse(item.notes); } catch (e) {}
        } else if (typeof item.notes === 'object' && item.notes !== null) {
          notesObj = item.notes;
        }

        const isEpPaid = item.event_profile?.payment_status === 'paid';
        const isNotesPaid = notesObj.payment_status === 'paid';

        if (isEpPaid || isNotesPaid) {
          counts[item.product_id] = (counts[item.product_id] || 0) + (item.quantity || 1);
        }
      });

      (profProdData || []).forEach(item => {
        counts[item.product_id] = (counts[item.product_id] || 0) + (item.quantity || 1);
      });
  ...
  const fetchSales = async (product) => {
    ...
    try {
      ...
      // 1. Fetch sales from event_profile_product (registration add-ons & bundle purchases)
      ...
      // Filter and map registrations & bundle sales
      const salesFromEp = (epProdData || [])
        .filter(item => {
          let notesObj = {};
          if (typeof item.notes === 'string') {
            try { notesObj = JSON.parse(item.notes); } catch (e) {}
        ...
        .map(item => {
          ...
          if (item.notes) {
            try {
              ...
              // Delete delivery & payment status info to clean custom field answers
              const cleanObj = { ...parsed };
              delete cleanObj.delivery_method;
              delete cleanObj.shipping_address;
              delete cleanObj.payment_status;
          ...
          const isBundle = !!customFieldsAnswers.is_bundle || !!customFieldsAnswers.bundle_id;
          ...
          return {
            ...
            buyer_name: customFieldsAnswers.nombre || item.event_profile?.full_name || item.event_profile?.profile?.name || 'Participante',
            ...
            type: isBundle ? 'Paquete / Bundle' : 'Registro / Add-on',
          };
        });
  ...
  return (
    <>
            ...
            <div className="mb-12">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h2 className="text-xl font-black text-amber-400 uppercase tracking-tight">
                  Paquetes & Kits Promocionales (Bundles)
                </h2>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {BUNDLES_DEFINITIONS.map((bundle) => {
                  const bundleItems = bundle.productIds
                    .map(id => products.find(p => p.id === id))
                    .filter(Boolean);

                  const regularTotalPriceCents = bundleItems.reduce((acc, p) => acc + (p.price_cents || 0), 0);
                  const discountedPriceCents = Math.round(regularTotalPriceCents * (1 - bundle.discountPercentage / 100));

                  // Calculate total buyers who bought at least one item of this bundle
                  const bundleSales = epSalesRaw.filter(item => {
                    let notesObj = {};
                    if (typeof item.notes === 'string') {
                      try { notesObj = JSON.parse(item.notes); } catch (e) {}
                    } else if (typeof item.notes === 'object' && item.notes !== null) {
                    ...
                    const isPaid = item.event_profile?.payment_status === 'paid' || notesObj.payment_status === 'paid';
                    const matchesBundle = notesObj.bundle_id === bundle.id || notesObj.is_bundle === true || bundle.productIds.includes(item.product_id);
                    return isPaid && matchesBundle;
                  });

                  // Unique buyers
                  const uniqueBuyersMap = new Map();
                  bundleSales.forEach(item => {
                    let parsed = {};
                    if (typeof item.notes === 'string') {
                      try { parsed = JSON.parse(item.notes); } catch (e) {}
                    } else if (typeof item.notes === 'object' && item.notes !== null) {
                      parsed = item.notes;
                    }
                    const bName = parsed.nombre || item.event_profile?.full_name || item.event_profile?.profile?.name || 'Participante';
                    const bEmail = item.event_profile?.profile?.email || '—';
                    const key = `${bName}_${bEmail}`;
                    if (!uniqueBuyersMap.has(key)) {
                      uniqueBuyersMap.set(key, {
                        name: bName,
                        email: bEmail,
                        itemsBought: [],
                      });
                    }
                    const prodName = bundleItems.find(p => p.id === item.product_id)?.title || item.product_id;
                    uniqueBuyersMap.get(key).itemsBought.push(prodName);
                  });

                  return (
                    <div
                      key={bundle.id}
                      className="bg-gradient-to-br from-gray-900 via-gray-900 to-amber-950/30 border border-amber-500/30 rounded-2xl p-6 shadow-xl relative overflow-hidden flex flex-col justify-between"
                    >
                      <div className="absolute top-0 right-0 bg-amber-500 text-gray-950 font-black text-xs px-4 py-1.5 rounded-bl-xl uppercase tracking-wider">
                        {bundle.discountPercentage}% OFF BUNDLE
                      </div>
                      ...
                      <div>
                        ...
                        <h3 className="text-xl font-bold text-white mt-1 mb-2">{bundle.title}</h3>
                        <p className="text-xs text-gray-400 mb-4 leading-relaxed">{bundle.description}</p>

                        <div className="bg-gray-950/80 border border-gray-800 rounded-xl p-4 mb-4">
                          <h4 className="text-xs font-bold text-gray-300 uppercase tracking-wider mb-2">
                            Artículos incluidos en el kit ({bundleItems.length}):
                          </h4>
                          <div className="space-y-2">
                            {bundleItems.map(item => (
                              <div key={item.id} className="flex items-center justify-between text-xs border-b border-gray-850/60 pb-1.5 last:border-0 last:pb-0">
                                <span className="text-gray-200 font-medium flex items-center gap-2">
                                  <span className="text-amber-400 font-bold">•</span> {item.title}
                                </span>
                                <span className="text-gray-400 font-mono">
                                  {centsToMoney(item.price_cents)}
                                </span>
                              </div>
                            ))}
                          </div>
                      ...
                      <div>
                        <div className="flex items-center justify-between border-t border-gray-800 pt-4 mb-4">
                          <div>
                            <span className="text-[10px] text-gray-400 block uppercase font-semibold">Precio Paquete</span>
                            <div className="flex items-baseline gap-2">
                              <span className="text-xl font-black text-emerald-400">{centsToMoney(discountedPriceCents)}</span>
                              <span className="text-xs text-gray-500 line-through">{centsToMoney(regularTotalPriceCents)}</span>
                            </div>
                          </div>
                          <div className="text-right">
                            <span className="text-[10px] text-gray-400 block uppercase font-semibold">Compradores Únicos</span>
                            <span className="text-lg font-black text-amber-400">{uniqueBuyersMap.size} Clientes</span>
                          </div>
                        </div>
                        ...
                        <button
                          ...
                          className="w-full bg-amber-500 hover:bg-amber-400 text-gray-950 font-black py-2.5 rounded-xl shadow-lg shadow-amber-500/10 hover:shadow-amber-500/20 transition duration-200 text-xs uppercase tracking-wider"
                        >
                          Ver Ventas de Paquete & Compradores
                        </button>
                      </div>
                    </div>
                  );
                    ...
                    <div className="flex-1 p-6">
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        <span className="text-[10px] font-bold bg-blue-950 text-blue-300 rounded px-2.5 py-1 border border-blue-900/50">
                          Ventas: {salesCounts[product.id] || 0} unid.
                        </span>
                        ...
                        {product.custom_fields_schema && (
                          <span className="text-[10px] font-bold bg-purple-950 text-purple-300 rounded px-2.5 py-1 border border-purple-900/50">
                            Custom fields
                          </span>
                        )}
                      </div>
      ...
      {/* BUNDLE SALES MODAL */}
      {showBundleModal && inspectingBundle && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/80" onClick={() => setShowBundleModal(false)} />
          <div className="relative w-full max-w-5xl bg-gray-900 border border-amber-500/30 rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col animate-in zoom-in-95 duration-200">
            {/* Modal Header */}
            <div className="px-6 py-4 border-b border-gray-800 flex justify-between items-center bg-gray-950">
              <div>
                <span className="text-[10px] font-bold tracking-widest text-amber-400 uppercase">
                  Ventas de Paquete Promocional
                </span>
                <h2 className="text-xl font-black text-white uppercase tracking-tight">
                  {inspectingBundle.title}
                </h2>
              </div>
              <button onClick={() => setShowBundleModal(false)} className="text-gray-400 hover:text-white">✕</button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {(() => {
                ...
                const bundleSales = epSalesRaw.filter(item => {
                  ...
                  const isPaid = item.event_profile?.payment_status === 'paid' || notesObj.payment_status === 'paid';
                  const matchesBundle = notesObj.bundle_id === inspectingBundle.id || notesObj.is_bundle === true || inspectingBundle.productIds.includes(item.product_id);
                  return isPaid && matchesBundle;
                });

                if (bundleSales.length === 0) {
                  return (
                    <div className="text-center p-12 text-gray-500">
                      No se han registrado ventas finalizadas para este paquete.
                    </div>
                  );
                }

                // Map sales per product in bundle
                const salesPerProduct = {};
                bundleItems.forEach(prod => {
                  salesPerProduct[prod.id] = bundleSales.filter(s => s.product_id === prod.id);
                });
                ...
                bundleSales.forEach(item => {
                  ...
                  if (!buyersMap.has(key)) {
                    buyersMap.set(key, {
                      name: bName,
                      email: bEmail,
                      delivery_method: parsed.delivery_method || item.notes?.delivery_method || 'pickup',
                      shipping_address: parsed.shipping_address || '',
                      created_at: item.created_at,
                      items: [],
                    });
                ...
                return (
                  <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div className="bg-gray-950 border border-gray-850 p-4 rounded-xl">
                        <span className="text-[10px] text-gray-400 font-bold uppercase">Compradores Únicos</span>
                        <h4 className="text-2xl font-black text-amber-400 mt-1">{uniqueBuyers.length} RIDER(S)</h4>
                      </div>
                      <div className="bg-gray-950 border border-gray-850 p-4 rounded-xl">
                        <span className="text-[10px] text-gray-400 font-bold uppercase">Total Artículos Entregables</span>
                        <h4 className="text-2xl font-black text-blue-400 mt-1">{bundleSales.length} Artículos</h4>
                      </div>
                      <div className="bg-gray-950 border border-gray-850 p-4 rounded-xl">
                        <span className="text-[10px] text-gray-400 font-bold uppercase">Monto Total Recaudado</span>
                        <h4 className="text-2xl font-black text-emerald-400 mt-1">
                          {centsToMoney(bundleSales.reduce((acc, s) => acc + (s.unit_price_cents || 0), 0))}
                        </h4>
                      </div>
                    </div>
                    ...
                    <div>
                      <h3 className="text-sm font-bold text-gray-200 uppercase tracking-wider mb-3">
                        Desglose de Ventas por Artículos del Kit
                      </h3>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                        {bundleItems.map(item => {
                          const count = (salesPerProduct[item.id] || []).length;
                          return (
                            <div key={item.id} className="bg-gray-950 border border-gray-800 p-3 rounded-xl">
                              <span className="text-xs font-semibold text-gray-300 line-clamp-1">{item.title}</span>
                              <div className="flex justify-between items-baseline mt-2">
                                <span className="text-lg font-black text-white">{count} unid.</span>
                                <span className="text-[10px] text-emerald-400 font-bold font-mono">
                                  {centsToMoney(count * (item.price_cents || 0))}
                                </span>
                              </div>
                            </div>
                          );
                    ...
                    <div>
                      <h3 className="text-sm font-bold text-gray-200 uppercase tracking-wider mb-3">
                        Lista de Clientes que compraron el Paquete ({uniqueBuyers.length})
                      </h3>
                      <div className="border border-gray-800 rounded-xl overflow-hidden bg-gray-950">
                        <table className="min-w-full text-xs text-left">
                          <thead className="bg-gray-900 border-b border-gray-800 text-gray-300 font-bold">
                            <tr>
                              <th className="px-4 py-3">Cliente / Rider</th>
                              <th className="px-4 py-3">Artículos Comprados</th>
                              <th className="px-4 py-3">Especificaciones (Tallas / Nombres)</th>
                              <th className="px-4 py-3">Entrega</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-gray-850 text-gray-300">
                            {uniqueBuyers.map((b, idx) => (
                              <tr key={idx}>
                                <td className="px-4 py-3">
                                  <span className="block font-bold text-white">{b.name}</span>
                                  <span className="text-[10px] text-gray-400">{b.email}</span>
                                </td>
                                <td className="px-4 py-3">
                                  <div className="flex flex-wrap gap-1">
                                    {b.items.map((it, i) => (
                                      <span key={i} className="text-[10px] font-bold bg-amber-950/40 text-amber-300 border border-amber-800/40 rounded px-2 py-0.5">
                                        {it.productTitle}
                                      </span>
                                    ))}
                                  </div>
                                ...
                                <td className="px-4 py-3 font-mono text-[10px]">
                                  {b.items.map((it, i) => {
                                    const detailsStr = Object.entries(it.details)
                                      .filter(([k]) => !['delivery_method', 'shipping_address', 'payment_status', 'is_bundle', 'bundle_id'].includes(k))
                                      .map(([k, v]) => `${k}: ${v}`)
                                      .join(', ');
                                    return detailsStr ? (
                                      <div key={i} className="text-gray-300">
                                        <span className="font-bold text-gray-400">{it.productTitle}:</span> {detailsStr}
                                      </div>
                                    ) : null;
                                  })}
                                </td>
                                <td className="px-4 py-3 whitespace-nowrap">
                                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                                    b.delivery_method === 'ship'
                                      ? 'bg-blue-900/30 text-blue-300 border border-blue-800/40'
                                      : 'bg-amber-900/30 text-amber-300 border border-amber-800/40'
                                  }`}>
                                    {b.delivery_method === 'ship' ? 'Envío' : 'Pickup'}
                                  </span>
                                  {b.shipping_address && (
                                    <div className="text-[10px] text-gray-400 max-w-[150px] truncate mt-0.5" title={b.shipping_address}>
                                      {b.shipping_address}
                                    </div>
                                  )}
                                </td>
            ...
            <div className="px-6 py-4 bg-gray-950 border-t border-gray-800 flex justify-end">
              <button
                ...
                className="bg-gray-900 hover:bg-gray-800 border border-gray-800 px-6 py-2 rounded-xl text-sm font-semibold transition"
              >
                Cerrar
              </button>
            </div>
