...
const Datatable = ({columns, data}) => {
    ...
    return (
        <>
            ...
            {!page.length && <div className='flex justify-center m-10 text-gray-600'><h4>No results found</h4></div>}
            ...
            <div className="flex flex-row w-full items-center justify-around lg:justify-between my-4">
                ...
                <span className="hidden lg:block">Page{' '}<b>{pageIndex + 1} of {pageOptions.length}</b>{' '}
        </span>

                <select
                    className="hidden lg:block form-select text-sm bg-white dark:bg-gray-800 dark:border-gray-800 outline-none shadow-none focus:shadow-none"
                    value={pageSize}
                    onChange={e => {
